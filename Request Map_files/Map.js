			...
			function readyfunc() { // The main function
				...
				if (!(typeof top.UserXCoordinate === 'undefined')) { // If we have the user's location
					...
					map = new Map("map", { // If we don't
						center: [-88.279, 42.675], // These are standard lat & long coordinates. Order goes: [Longitude, Latitude]
					...
					$.getJSON($('#RESTAPI').html()+'/Requests.svc/?format=json&coordsys=mercator&statusflag=open&take=500', function(data) { // Get the 500 most recent open requests
						...
						$.getJSON($('#RESTAPI').html()+'/Requests.svc/?format=json&coordsys=mercator&statusflag=closed&statusdays=30&take=250', function(data) { // Get the 250 most recent closed requests
							...
							process = function(dat, col) {
								...
								var pArray = []; // The array of requests (used to match indices)
								var pIndex = 0; // The index of the request (matched along with pArray)
								...
								$.each(dat, function() { // We need to check each returned piece of data to see if it has coordinates already
									...
									function locate() { // Get ready to locate
										this.BuildingNumber = "";
										this.Street = "";
										this.Street2 = "";
										this.LocationCity = "";
										...
										if ((p.BuildingNumber != null)&&(p.BuildingNumber != "")) // We only want to add the spaces and symbols if there is something provided
											this.BuildingNumber = p.BuildingNumber + " ";
										...
										if ((p.LocationState != null)&&(p.LocationState != ""))
											this.LocationState = p.LocationState + " ";
										if ((p.LocationZipCode != null)&&(p.LocationZipCode != ""))
											this.LocationZipCode = p.LocationZipCode;
										  p.address = {
											"SingleLine": this.BuildingNumber + this.Street + this.Street2 + this.LocationCity + this.LocationState + this.LocationZipCode // Build the address as best as possible
										  };
										locator.outSpatialReference = map.spatialReference;
										var options = {
											address: p.address,
											outFields: ["Loc_name", "" + pIndex] // Loc_name isn't used in this code. pIndex is how we keep track of which async is handled first
										}
										pIndex++;
										locator.addressToLocations(options); // Get the options
									}
									
									locate();
									pArray.push(p); // Build the array of objects to match with later
									
									showResults = function(evt) {
										dataCounter++; // Keep track of how many asyncs have come back, we don't want to move onto the next step until this is finished
										var ind = 0; // The index we will be using (the proper pIndex from before)
										var candidate;
										arrayUtils.every(evt.addresses, function(candidate) {
											if (candidate.score > 80) { // If it is a likely candidate
												for (var i = 0; i<jsonDataCount; i++) { // Cycle through the indices
													if (typeof candidate.attributes["" + i] != 'undefined')  // If this is the proper index
														ind = i; // We will be using this index
												}
												pArray[ind].XCoordinate = candidate.location.x; // Set the X coordinate for the proper request
												pArray[ind].YCoordinate = candidate.location.y; // Set the Y coordinate for the proper request
									...
									locator.on("address-to-locations-complete", showResults); // Once we have the candidates, get ready to update the fetched json data so we can add all points to the map
								});
								
								if (jsonDataCount === 0) { // Still need to 'add clusters', even if there are no clusters to add
									addClusters(dat, col);
								}
							}
			    ...
			    function addClusters(resp, col) { // Add the cluster layers. The data is resp, the color we want to use is col
					...
					openRequestInfo.data = arrayUtils.map(resp, function(p) {
						if (p.XCoordinate != null) { // If we have coordinates
							...
							var date = new Date(p.StatusDate);
